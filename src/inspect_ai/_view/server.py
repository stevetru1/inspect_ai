...
def log_listing_response(logs: list[EvalLogInfo], log_dir: str) -> web.Response:
    # Normalize file names for Azure: when the user supplied az:// the underlying
    # filesystem may return abfs:// or abfss:// URIs. The frontend code expects
    # file names to share a prefix with the provided log_dir; mismatched schemes
    # cause the row to be filtered out client-side. Rewrite schemes to match
    # log_dir when safe.
    ...
    response = dict(
        ...
        files=[
            dict(
                ...
                size=log.size,
                # Some remote providers may not supply mtime; use 0 so UI can sort
