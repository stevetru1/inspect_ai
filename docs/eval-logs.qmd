...
Use `--fs-option key=value` to pass extra `adlfs` options if required (e.g. tuning timeouts). For web deployment (App Service / Container Apps), just replicate the same environment variable setup; managed identity remains the most secure pattern.
